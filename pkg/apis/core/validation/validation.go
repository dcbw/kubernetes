...
func ValidateNode(node *core.Node) field.ErrorList {
	...
	if len(node.Spec.PodCIDRs) != 0 {
		...
		for idx, value := range node.Spec.PodCIDRs {
			...
			if err != nil {
				allErrs = append(allErrs, field.Invalid(field.NewPath("spec", "podCIDRs"), node.Spec.PodCIDRs, fmt.Sprintf("not a valid CIDR[%v]=%v", idx, value)))
			}
			// should not be duplicate
			if atIdx, ok := seen[value]; ok {
				allErrs = append(allErrs, field.Invalid(field.NewPath("spec", "podCIDRs"), node.Spec.PodCIDRs, fmt.Sprintf("duplicate cidr %v duplicate cidr at index:%v", value, atIdx)))
			} else {
				seen[value] = idx
			}
		}
