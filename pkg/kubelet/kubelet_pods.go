...
func makeMounts(pod *v1.Pod, podDir string, container *v1.Container, hostName, hostDomain string, podIPs []string, podVolumes kubecontainer.VolumeMap, mounter mountutil.Interface, subpather subpath.Interface, expandEnvs []kubecontainer.EnvVar) ([]kubecontainer.Mount, func(), error) {
	// Kubernetes only mounts on /etc/hosts if:
	// - container is not an infrastructure (pause) container
	// - container is not already mounting on /etc/hosts
	// - OS is not Windows
	// Kubernetes will not mount /etc/hosts if:
	// - when the Pod sandbox is being created, its IP is still unknown. Hence, PodIPs will not have been set.
...
func ensureHostsFile(fileName string, podIPs []string, hostName, hostDomainName string, hostAliases []v1.HostAlias, useHostNetwork bool) error {
...
// information.
func managedHostsFileContent(podIPs []string, hostName, hostDomainName string, hostAliases []v1.HostAlias) []byte {
	var buffer bytes.Buffer
	...
	for _, ip := range podIPs {
		if len(hostDomainName) > 0 {
			buffer.WriteString(fmt.Sprintf("%s\t%s.%s\t%s\n", ip, hostName, hostDomainName, hostName))
		} else {
...
func (kl *Kubelet) GenerateRunContainerOptions(pod *v1.Pod, container *v1.Container, podIPs []string) (*kubecontainer.RunContainerOptions, func(), error) {
	...
	envs, err := kl.makeEnvironmentVariables(pod, container, podIPs)
	if err != nil {
		return nil, nil, err
	}
	opts.Envs = append(opts.Envs, envs...)

	mounts, cleanupAction, err := makeMounts(pod, kl.getPodDir(pod.UID), container, hostname, hostDomainName, podIPs, volumes, kl.mounter, kl.subpather, opts.Envs)
	if err != nil {
		return nil, cleanupAction, err
	}
...
func (kl *Kubelet) makeEnvironmentVariables(pod *v1.Pod, container *v1.Container, podIPs []string) ([]kubecontainer.EnvVar, error) {
	...
	for _, envVar := range container.Env {
		...
		if runtimeVal != "" {
			...
			case envVar.ValueFrom.FieldRef != nil:
				runtimeVal, err = kl.podFieldSelectorRuntimeValue(envVar.ValueFrom.FieldRef, pod, podIPs)
				if err != nil {
					return result, err
				}
...
// selector for a pod.
func (kl *Kubelet) podFieldSelectorRuntimeValue(fs *v1.ObjectFieldSelector, pod *v1.Pod, podIPs []string) (string, error) {
