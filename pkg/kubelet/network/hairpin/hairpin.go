...
func SetUpContainer(containerPid int, containerInterfaceName string) error {
	hostIfName, err := findPairInterfaceOfContainerInterfaceSysfs(exec.New(), containerPid, containerInterfaceName)
	if err != nil {
		hostIfName, err = findPairInterfaceOfContainerInterfaceSysfs(exec.New(), containerPid, containerInterfaceName)
		if err != nil {
			return err
...
func getPeerIfindex(e exec.Interface, containerPid int, containerInterfaceName string, regex *regexp.Regexp, nsenterArgs ...string) (string, error) {
	...
	output, err := e.Command(nsenterPath, "-t", fmt.Sprintf("%d", containerPid), "-n", "-F", "--", nsenterArgs...).CombinedOutput()
	if err != nil {
