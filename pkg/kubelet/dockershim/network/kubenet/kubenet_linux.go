...
func NewPlugin(networkPluginDirs []string) network.NetworkPlugin {
	...
	return &kubenetNetworkPlugin{
		...
		//TODO (khenidak): maybe it should be defaulted to zero address?
		// 10.0.0.0/8 is the default value before dualstack changes
...
func (plugin *kubenetNetworkPlugin) Init(host network.Host, hairpinMode kubeletconfig.HairpinMode, nonMasqueradeCIDRs string, mtu int) error {
	plugin.host = host
	plugin.hairpinMode = hairpinMode
	plugin.nonMasqueradeCIDRs = strings.Split(nonMasqueradeCIDRs, ",")
...
func (plugin *kubenetNetworkPlugin) ensureMasqRule() error {
	for _, currentCIDR := range plugin.nonMasqueradeCIDRs {
		if currentCIDR != zeroCIDRv4 && currentCIDR != zeroCIDRv6 {
			// switch according to target nonMaqureadeCidr ip family
			ipt := plugin.iptables
			if netutils.IsIPv6CIDRString(currentCIDR) {
	...
	//TODO: khenidak
	// We are masquerading the secondary address for dual stack here
	// We want to avoid mirroring everything kube-proxy does for two address
	// family, just to get the masquerading function.
	// REMOVE - once kube-proxy talks dual stack
	for idx, currentCIDR := range plugin.podCIDRs {
		if 0 < idx {
			cidrString := currentCIDR.String()
			ipt := plugin.iptables
			if netutils.IsIPv6CIDRString(cidrString) {
				ipt = plugin.iptablesv6
			}

			if _, err := ipt.EnsureRule(utiliptables.Append, utiliptables.TableNAT, utiliptables.ChainPostrouting,
				"-m", "comment", "--comment", "kubenet: SNAT for secondary dual stack ips",
				"-s", cidrString,
				"!", "-d", cidrString,
				"-j", "MASQUERADE"); err != nil {
				return fmt.Errorf("Failed to maquerading secondary cidr:%v with error %v", cidrString, err)
			}
			klog.V(4).Infof("kubenet masqueraded secondary cidr:%v", cidrString)
		}
	}
	return nil
}
...
func (plugin *kubenetNetworkPlugin) Event(name string, details map[string]interface{}) {
	...
	if err != nil {
		klog.V(2).Infof("** failed to set up CNI with %v err:%v", json, err)
		...
		plugin.clearBridgeAddressesExcept()
	}
}

// clear all address on bridge except those operated on by kubenet
func (plugin *kubenetNetworkPlugin) clearBridgeAddressesExcept() {
...
func (plugin *kubenetNetworkPlugin) setup(namespace string, name string, id kubecontainer.ContainerID, annotations map[string]string) error {
	...
	//TODO: v1.16 (khenidak) modify cni types
	// to provide list of ips instead 2 (v4 and v6).
	// this will make all further call operate on a list of ips
	if res.IP4 != nil {
...
func (plugin *kubenetNetworkPlugin) addTrafficShaping(id kubecontainer.ContainerID, annotations map[string]string) error {
	...
	if egress != nil || ingress != nil {
		for _, ip := range iplist.UnsortedList() {
			mask := 32
...
func (plugin *kubenetNetworkPlugin) addPortMapping(id kubecontainer.ContainerID, name, namespace string) error {
	...
	if nil == portMappings || 0 == len(portMappings) {
		return nil
	}
