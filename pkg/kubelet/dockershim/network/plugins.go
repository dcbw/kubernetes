...
type PodNetworkStatus struct {
	...
	// IPs are list of additiopnal ip assigned to pod
	IPs []net.IP `json:"ips" description:"list of additional ips assigned to pod"`
}
...
func GetPodIPs(execer utilexec.Interface, nsenterPath, netnsPath, interfaceName string) ([]net.IP, error) {
	if !utilfeature.DefaultFeatureGate.Enabled(kubefeatures.IPv6DualStack) {
		...
		list := make([]net.IP, 0)
		var err4, err6 error
		if ipv4, err4 := getOnePodIP(execer, nsenterPath, netnsPath, interfaceName, "-4"); nil != err4 {
			list = append(list, ipv4)
		}

		if ipv6, err6 := getOnePodIP(execer, nsenterPath, netnsPath, interfaceName, "-6"); nil != err6 {
