...
import (
	...
	// Azure route controller changes behavior
	// if ipv6dual stack feature is turned on
...
func (az *Cloud) CreateRoute(ctx context.Context, clusterName string, nameHint string, kubeRoute *cloudprovider.Route) error {
	...
	if !utilfeature.DefaultFeatureGate.Enabled(IPv6DualStack) {
		...
		bCIDRv6 := utilnet.IsIPv6CIDRString(string(kubeRoute.DestinationCIDR))
		...
		targetIP, err = findFirstIPByFamily(nodePrivateIPs, bCIDRv6)
		if nil != err {
